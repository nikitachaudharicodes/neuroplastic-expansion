...
from modules_SAC import SACActor, MLPCritic
...
class SAC(object):
	def __init__(
		...
		# Neural networks
		self.actor = SACActor(args.state_dim, args.action_dim, args.max_action, args.hidden_dim).to(device)
	...
	def train(self, replay_buffer:ReplayBuffer, batch_size=256):
		...
		with torch.no_grad():
			...
			with torch.no_grad():
				next_action, next_log_prob = self.actor_target.sample(nstep_next_state)

			# Compute the target Q value
			target_Q1, target_Q2 = self.critic_target(nstep_next_state, next_action)
			target_Q = torch.min(target_Q1, target_Q2) - self.alpha * next_log_prob  # added entropuy for sac
			if current_nstep == 1:
		...
		if self.total_it % self.policy_freq == 0:
			action, log_prob = self.actor.sample(state[:,0])
			actor_loss = (self.alpha * log_prob - self.critic.Q1(state[:,0], action)).mean()
	...
	def train_rej(self, replay_buffer: ReplayBuffer, batch_size=256):
		...
		with torch.no_grad():
			...
			modified_n = modified_n.type(torch.long)
			nstep_next_state = next_state[np.arange(batch_size), modified_n[:, 0]]
			next_action, next_log_prob = self.actor_target.sample_rej(nstep_next_state)
			# Compute the target Q value
		...
		# Delayed policy updates
		if self.total_it % self.policy_freq == 0:
			action, log_prob = self.actor.sample_rej(state[:, 0])
			actor_loss = (self.alpha * log_prob - self.critic.Q1_rej(state[:, 0], action)).mean()
			# Optimize the actor
			self.actor_optimizer.zero_grad()
			actor_loss.backward()
			end_grow = show_sparsity(self.actor.state_dict(), to_print=False) <= self.actor_sparsity
