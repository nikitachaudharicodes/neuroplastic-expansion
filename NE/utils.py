...
class ReplayBuffer(object):
    ...
    def recall_sample(self, batch_size, nstep=1):
        range_ = self.hist_size()
        ind = (np.random.randint(0, int(range_) - nstep + 1, (batch_size,1)) + self.left_ptr + np.arange(nstep))%self.max_size
        return (
            torch.FloatTensor(self.state[ind]).to(self.device),
            torch.FloatTensor(self.action[ind]).to(self.device),
            torch.FloatTensor(self.next_state[ind]).to(self.device),
    ...
    def hist_size(self):
        range_ = ((self.ptr+self.max_size-self.left_ptr)%self.max_size)*0.3
        return range_
